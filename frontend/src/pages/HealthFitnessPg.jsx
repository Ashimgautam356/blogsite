import React from 'react'
import test13 from '../images/test13.jpg'
import test10 from '../images/test10.jpg'


const HealthFitnessPg = () => {
  return (
    <div className='pt-16 relative flex flex-col items-center w-full'>
        <div className='font-bold text-5xl center'>
            <h1>Health And Fitness</h1>
        </div>
        <div className='font-medium text-lg p-4 w-4/6 sm:w-5/6'>
            <h2>Your complete playbook to a longer, stronger life—featuring professional workout advice, celebrity training secrets, nutrition tips, and more.</h2>
        </div>
        <div className='w-full relative flex flex-row justify-center mb-10'> 
            <img src={test13} alt="" className='w-5/6'/>
        </div>

        <div className='relative flex flex-col items-center sm:w-5/6 p-5 w-4/6 h-3/6 mb-20'>
        <div className='grid grid-cols-2'>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            <div className='flex flex-row justify-between  md:w-4/6  col-span-2 relative h-fit border-b border-black mb-6 pb-3'>
                <div className='min-h-full w-4/6 sm:w-2/6 relative'>
                    <img src={test10} alt="" className='min-h-full w-5/6  sm:w-full   object-cover object-center'/>
                </div>
                <div className=' sm:w-4/6 mr-2  sm:pl-3'>
                    <h1 className='font-bold text-2xl pb-2'>This is the heading of the above images</h1>
                    <h2 className=''>This is the sub heading of the above imges also of this heading.</h2>
                </div>
            </div>
            
        </div>
        <div className='w-1/5 bg-black text-white text-center p-3 shadow-mine'>
            <button>See More</button>
        </div>
    </div>
    </div>
  )
}

export default HealthFitnessPg